...
Namespace CustomControls_TitledLabel
	...
	Public Class XrTitledLabel
		...
		Protected Overrides Sub SerializeProperties(ByVal serializer As XRSerializer)
			...
			serializer.SerializeString("TitleText", titleText_Renamed, "")
			serializer.SerializeFont("TitleFont", Me.TitleFont)
			serializer.SerializeBoolean("DisplayTitle", Me.DisplayTitle)
			serializer.SerializeEnum("ContextTextAlignment", Me.ContextTextAlignment)
			serializer.SerializeEnum("TitleTextAlignment", Me.TitleTextAlignment)
		End Sub

		Protected Overrides Sub DeserializeProperties(ByVal serializer As XRSerializer)
			MyBase.DeserializeProperties(serializer)
			Me.TitleText = serializer.DeserializeString("TitleText", "")
			Dim defaultFont As New Font("Times New Roman", 6.75F, FontStyle.Regular, GraphicsUnit.Point, (CType(0, Byte)))
			Me.TitleFont = serializer.DeserializeFont("TitleFont", defaultFont)
			Me.DisplayTitle = serializer.DeserializeBoolean("DisplayTitle", True)
			Me.ContextTextAlignment = CType(serializer.DeserializeEnum("ContextTextAlignment", GetType(ContextTextAlignmentEnum), ContextTextAlignmentEnum.BelowTitleText), ContextTextAlignmentEnum)
			Me.TitleTextAlignment = CType(serializer.DeserializeEnum("TitleTextAlignment", GetType(TitleTextAlignmentEnum), TitleTextAlignmentEnum.TopLeft), TitleTextAlignmentEnum)
		End Sub
